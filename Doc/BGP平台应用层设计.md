...
施工进度模拟由一系列时间点及相对应的BIM信息描述。

**施工数据**采用数据库形式存储，支持EXCEL和JSON格式导入，施工数据表示例如下

| DT         | Content     | Spend(Unit:10000￥) | Staff |
| ---------- | ----------- | ------------------- | ----- |
...
要实现一个模拟过程（如施工过程ProcessConstruction），应当子类化Process，并按时间序列存储施工数据，以备execute调用。此外还应该记录已执行execute的时间进度，以便下次调用execute时根据时间判断应该前进还是后退。
